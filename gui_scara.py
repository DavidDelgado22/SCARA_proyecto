...
Características:
- Estilo oscuro profesional con mejor contraste y tipografía distinta.
- Contenedores “suavizados” (menos cuadrados) con padding y bordes sutiles.
...
class ScaraGUI:
    ...
    def on_enviar_arduino(self):
        ...
        line = f"{q1_rad:.4f},{q2_rad:.4f},{d3_m:.4f},{g}\n"
        try:
    ...
    def on_add_pose_to_routine(self):
        ...
        self.routine.append(
            {"q1_deg": q1_deg, "q2_deg": q2_deg, "d3_mm": d3_mm}
        )
        idx = len(self.routine)
        self.tree_routine.insert(
            "", tk.END, values=(idx, f"{q1_deg:.1f}", f"{q2_deg:.1f}", f"{d3_mm:.1f}")
        )
        self._log(
            f"[RUTINA] Añadida pose #{idx}: "
            f"q1={q1_deg:.1f}°, q2={q2_deg:.1f}°, d3={d3_mm:.1f} mm"
        )
    ...
    def on_simulate_routine(self):
        ...
        try:
            delay_ms = int(self.routine_delay_var.get())
        except ValueError:
    ...
    def on_execute_routine(self):
        ...
        def step(i):
            ...
            if self.last_result is not None:
                ...
                try:
                    self.ser.write(line.encode("ascii"))
                    self._log(f"[RUTINA][TX] Paso {i+1}/{len(self.routine)}: {line.strip()}")
                    self._append_serial_monitor(line.strip(), prefix="TX")
                except Exception as e:
                    self._log(f"[ERROR] Falló envío en paso {i+1}: {e}")
